...
public class UserService {
    ...
    public boolean checkUserNameAvailability(String username) {
        return users.stream()
                .anyMatch(userName -> userName.getUserName().equalsIgnoreCase(username));
    }
    ...
    public boolean authenticate(String username, String password) {
        return users.stream()
            .filter(user -> user.getUserName().equalsIgnoreCase(username)) // Filter by usernames equal to username.
            .findFirst() // Ask to return the first that is founded.
